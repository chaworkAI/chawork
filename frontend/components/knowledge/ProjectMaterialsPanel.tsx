...
export function ProjectMaterialsPanel() {
  ...
  const handleFileSelect = useCallback(async () => {
    const filterName = getLabel("import.dialog.supported_files", "支持的文档")
  ...
  return (
    ...
    >
      <section
        ...
        role="dialog"
        aria-modal="true"
        aria-label={getLabel("knowledge.project_materials", "项目资料")}
        ...
        <header className="flex shrink-0 items-start justify-between gap-4 border-b border-line-soft px-[22px] py-5">
          <div className="min-w-0">
            <p className="m-0 text-[12px] font-extrabold uppercase text-[var(--subtle)]">
              {getLabel("knowledge.workspace_kicker", "工作区")}
            </p>
            <h2 className="mt-[3px] text-[21px] font-extrabold text-ink">
              {getLabel("knowledge.project_materials", "项目资料")}
            </h2>
            <p className="mt-1 text-[13px] text-muted-foreground">
              {getLabel(
                "knowledge.project_materials_description",
          ...
          <Button
            ...
            aria-label={getLabel("knowledge.close_project_materials", "关闭项目资料")}
            onClick={close}
          >
            <X className="size-4" />
          </Button>
        ...
        <div className="grid min-h-0 flex-1 grid-cols-[320px_minmax(0,1fr)] gap-0 max-[760px]:grid-cols-1">
          <aside className="min-h-0 overflow-y-auto border-r border-line-soft p-[22px] max-[760px]:border-r-0 max-[760px]:border-b">
            <section>
              <h3 className="text-[12px] font-semibold text-ink">
                {getLabel("knowledge.import_materials", "导入资料")}
              </h3>
              <div
                ref={dropRef}
                onDrop={handleDrop}
              ...
              >
                <p className="text-[12px] text-muted-foreground">
                  {getLabel("knowledge.drop_or_choose", "拖入文件，或选择本地文档")}
                </p>
                <Button
                  type="button"
                  size="sm"
                ...
                >
                  {getLabel("import.choose_file", "选择文件")}
                </Button>
              ...
              {isImporting ? (
                <div className="mt-3 rounded-[13px] border border-warning/20 bg-warning/10 px-3 py-2 text-[12px] text-warning">
                  {getLabel("knowledge.import_running", "有导入任务正在运行")}
                </div>
              ) : null}
            </section>
            ...
            <section className="mt-5">
              <h3 className="text-[12px] font-semibold text-ink">
                {getLabel("knowledge.import_tasks", "导入任务")}
              </h3>
              <div className="mt-2">
          ...
          <main className="flex min-h-0 flex-col overflow-hidden p-[22px]">
            <div className="mb-3 flex shrink-0 items-center justify-between gap-3">
              <h3 className="text-[12px] font-semibold text-ink">
                {getLabel("knowledge.search_materials", "搜索资料")}
              </h3>
